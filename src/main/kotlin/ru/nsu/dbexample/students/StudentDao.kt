...
import ru.nsu.dbexample.groups.ActualGroup
import ru.nsu.dbexample.groups.Group
import ru.nsu.dbexample.groups.GroupProxy
import ru.nsu.dbexample.groups.createProxy
...
class StudentDao(private val dataSource: DataSource) {
    ...
    fun getStudents(page: Page): List<Student> {
        ...
        val rs = stmt.executeQuery()
        while (rs.next()) {
            val groupId = rs.getLong("student_group")
            res.add(ActualStudent(
                    rs.getLong("id"), rs.getString("full_name"),
                    createProxy<Group> {
                        val stmt2 = conn.prepareStatement("SELECT * FROM groups WHERE id = ?")
                        stmt2.setLong(1,groupId )
                        val rs2 = stmt2.executeQuery()
                        if (rs2.next()) {
                            ActualGroup(groupId, rs.getString("number"), rs.getInt("students_count"))
                        } else {
                            throw IllegalStateException("Group with id $groupId not found")
                        }
                    }
            ))
        }
